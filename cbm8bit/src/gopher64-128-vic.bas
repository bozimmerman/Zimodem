...
17 P$="ok":POKE186,PEEK(254):BA=1200:XB=1200:TB$=CHR$(7)
...
101 REM
102 PRINT:PRINT"{clear}{down}{left}";:NC=POS(0):NL=23:IFSY=34THENNL=22
110 REM
...
320 PRINT"URL: gopher://";:GOSUB9500:UR$=P$:IFUR$=""THENEND
330 IFVAL(RIGHT$(UR$,1))=0ANDRIGHT$(UR$,1)<>"0"THENUR$=UR$+":70"
400 DIMBU$(30,1):BU$(0,0)=UR$:BU$(0,1)="":BU=0
410 DIMPA$(100):DIMPA(4):DIMSC$(25):DIMSC(25):REM top,bot,showtop,numlink
500 GOTO2000:REM -- GO BEGIN!
...
1300 P$=UR$:GOSUB600:FORI=0TO4:PA(I)=0:NEXT:PT=0
1310 IFPEEK(DD)AND16=0THEN1390
1320 GOSUB930:IFP$=""THENPT=PT+1:IFPT>10THEN1390
1330 PT=0:PA$(PA(1))=P$:PA(1)=PA(1)+1:IFPA(1)>100THEN1390
1340 GOTO1310
1390 PRINT#5,"ath0":GOSUB9000:RETURN
1999 REM REQUEST PAGE, BUILD IT
2000 UR$=BU$(BU,0):GOSUB1000:IFE>0THEN300
2010 UR$=BU$(BU,1):GOSUB1300
2100 PL=PA(2):SL=0:FORI=0to25:SC$(I)="":SC(I)=-1:NEXT
2110 PL=PL+1:IFSL>=NLORPL>=PA(1)THEN2500
2120 P$=PA$(PL):LS$=LEFT$(P$,1):LS=VAL(LS$):P$=MID$(P$,2)
2130 IFLS$="0"ORLS=1ORLS=7THENSC(SL)=PL
2140 GOSUB9200:SC$(SL)=PN$(0)
2190 SL=SL+1:GOTO2110
2500 SE=-1:SF=0:FORI=0TONL:IFSC(I)>=0ANDSE=-1THENSE=I
2510 NEXT
2600 PRINT"{clear}";
2610 FORI=1TONL:IFI=SETHENPRINT"{reverse on}";
2620 IFSF=0THENPRINTLEFT$(SC$(I),NC):GOTO2650
2625 L=LEN(SC$(I)):IFL<SFTHENPRINT:GOTO2620
2630 IFL<SF+NCTHENPRINTMID$(SC$(I),SF,L-SF):GOTO2620
2635 PRINTMID$(SC$(I),SF)
2650 IFI=SETHENPRINT"{reverse off}"
2660 NEXT
2699 REM -- MAIN MENU
...
3000 I=SE-1
3010 IFI<0THEN3050
3020 IFSC(I)>=0THENSE=I:GOTO2600
3030 I=I-1:GOTO3010
...
3100 I=SE+1
3110 IFI>=NLTHEN3150
3120 IFSC(I)>=0THENSE=I:GOTO2600
3130 I=I+1:GOTO3110
3150 IFPL>=PA(1)THEN2700
3160 PA(2)=PL:GOTO2100
3200 SF=0THEN2700
...
9250 RETURN
9300 PRINT"{up}";CHR$(128+32);:FORI=1TONC:PRINT" ";:NEXT
9310 PRINT"{up}";CHR$(128+32);:RETURN
