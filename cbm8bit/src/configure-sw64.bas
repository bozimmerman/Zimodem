!--------------------------------------------------
!- Tuesday, August 17, 2021 9:44:38 AM
!- Import of : 
!- z:\lost+found\configure.prg
!- Commodore 64
!--------------------------------------------------
1 REM CONFIGURE64SW  19200B 2.0+
2 REM UPDATED 08/15/2021 12:54P
5 POKE254,PEEK(186):IFPEEK(254)<8THENPOKE254,8
10 GOSUB9100:IFSY=61THENPOKE58,254:CLR
...
30 CO$="{light blue}":IFSY=226ORSY=227THENML=49152:POKE665,73-(PEEK(678)*30)
35 IFSY=34THENML=22273:IFPEEK(ML)<>76THENCLOSE5:LOAD"pmlvic.bin",PEEK(254),1:RUN
38 IFSY=34THENPOKE36879,27:CO$=CHR$(31)
39 IFSY=226ANDPEEK(ML+1)<>209THENCLOSE5:LOAD"pml64.bin",PEEK(254),1:RUN
40 IFSY=227ANDPEEK(ML+1)<>209THENCLOSE5:LOAD"pml64.bin",PEEK(254),1:RUN
45 IFSY=227ANDPEEK(51201)<>3THENCLOSE5:LOAD"swiftdrvr.bin",PEEK(254),1:RUN
...
1070 SYSML+12:PRINT#5,CR$;"atc";QU$;"198.251.68.243:80";QU$;CR$;
